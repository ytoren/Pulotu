...
read_tsv('https://ndownloader.figshare.com/files/2161625') %>%
    filter(substring(Culture, 1, 8) != 'Malagasy') -> d
```
...
I ended up choosing a coordinate manually (140, -10). The following code is not particularly "tidy" but works with the ```ggmap``` package (@ggmap_cite) to show the relative population of the culture on a Google map:
